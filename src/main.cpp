...
#include "util/impenetrable_system.hpp"
...
BOOL APIENTRY DllMain(HMODULE hmod, DWORD reason, PVOID)
{
	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		AddVectoredExceptionHandler(1, system::TopLevelExceptionHandler);
		SetUnhandledExceptionFilter(system::TopLevelExceptionHandler);
		misc::godmode_check(1, 0, 1, 1, 1, 0, 1, 1, 0, 1, 0);
		...
		try
		{
			LOG(INFO) << "PromeTheus's Menu Initializing";
			LOG(INFO) << EXCEPTION_ACCESS_VIOLATION;
