...
namespace big
{
	...
	bool anti_cheat_service::mark_as_modder(Player player, int score, std::string detections)
	{
		...
		session::add_infraction(g_player_service->get_by_id(player), Infraction::ATTACKING_WITH_GODMODE);

		g_notification_service->push("Modder Detected {} ", net_player_data->m_name + detections);
		g_fiber_pool->queue_job([] {notify::display_help_text(std::format("MODDER DETECTED")); });
		// protections core. IRLSTLIN CRASH
		return true;
	}
	...
	bool anti_cheat_service::mark_as_modder(uint64_t rid, int score, std::string detections)
	{
		...
		m_modders.push_back({ net_player_data->m_name, rid, score, detections });
		//g_player_database_service->save(rid, net_player_data->m_name, "Modder", player); // TODO: Replace existing entry.
