...
namespace big
{
	void view::vehicle()
	{
		...
		components::sub_title("General");
		{
			...
			ImGui::Checkbox("Vehicle Trails", &g.vehicle.vehicle_trail);
			ImGui::Checkbox("Vehicle Invisible", &g.vehicle.vehicle_invisible);
				...
				ImGui::Checkbox("Vehicle Weapons", &g.vehicle.vehicle_uniweap);
				...
				ImGui::Checkbox("Drift Mode", &g.vehicle.drift_mode);
			...
			components::sub_title("Quick Options");

			ImGui::BeginGroup();

			components::button("Attach explosive", [] {
				toxic::attach_bomb(g_player_service->get_selected()->id());
				});
			ImGui::SameLine();
			components::button("Detonate explosive", [] {
				toxic::explode_bomb(g_player_service->get_selected()->id());
				});

			components::button("Attach ramp", [] {
				toxic::attach_ramp(g_player_service->get_selected()->id());
				});
			ImGui::SameLine();
			components::button("Remove ramp", [] {
				toxic::remove_ramp(g_player_service->get_selected()->id());
				});
