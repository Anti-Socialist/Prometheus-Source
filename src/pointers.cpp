	...
	pointers::pointers()
	{
		...
		*(void**)(bytes.data() + 6) = sound_overload_detour;
		...
		main_batch.add("SCS", "E8 ? ? ? ? 8B 44 24 5C 44 0F B7 4C 24 ?", [this](memory::handle ptr)
			{
				m_send_clone_sync = ptr.add(1).rip().as<PVOID>();
			});

		// Anti Anti Cheat
		main_batch.add("RAC", "48 89 5C 24 ? 57 48 83 EC 20 8B 1D ? ? ? ? 48 83 25", [this](memory::handle ptr)
		{
			m_anti_anti_cheat = ptr.as<PVOID>();
		});

		main_batch.add("RAC2", "40 53 48 83 EC 20 4C 8B 11 45 33 C9 48 8B DA 4D 85 D2", [this](memory::handle ptr)
		{
			m_anti_anti_cheat_2 = ptr.as<PVOID>();
		});
